...
void Scurve::runtest(){
    ...
    std::cout<<"traject displacement:"<<r.tr_cs<<std::endl;
    std::cout<<"traject total time:"<<r.tr_ct<<std::endl;

    for(double t=0; t<r.tr_ct; t+=0.01){
        r=motion<double>(s, vo, ve, vs, a, t, gain);

        std::cout<<"scurve displacement result:"<<r.sc_sr<<std::endl;
        std::cout<<"scurve acceleration result:"<<r.sc_ar<<std::endl;
        std::cout<<"scurve velocity result:"<<r.sc_vr<<std::endl;
    }
}
...
template<typename T>
Result<T> Scurve::scurve_fwd(T gain, T am, T vo, T ve, T at_time){
    Result<T> r;
...
Result<T> scurve_bck(T gain, T am, T vo, T ve, T at_time){
    ...
    if(t<t1){                           //! Period concave t1
        ...
        r.sc_ar=-abs(jm*t);        //! Invert acceleration.
        return r;
    }
    if(t>=t1 && t<=t1+t2){              //! Period lineair transition t2
        t-=t1;
        ...
        r.sc_ar=-abs(as);          //! Invert acceleration.
        return r;
    }
    if(t>t1+t2 && t<=t1+t2+t3){          //! Period convex t3
        t-=t1;
        ...
        r.sc_ar=-abs(as-jm*t);     //! Invert acceleration.
        return r;
    }
    //! Normally don't get here.
    return r;
...
//! gain=curve power.
template<typename T>
Result<T> Scurve::motion(T s, T vo, T ve, T vs, T a, T at_time, T gain){
